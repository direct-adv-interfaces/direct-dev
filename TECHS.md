...
Технология формирует пустой файл с результатами тестов в формате технологии [enb-phantom-testing](https://www.npmjs.com/package/enb-phantom-testing). При создании можно передать параметр `needCoverage` - если передать значение `true`, в результирующий файл будет добавлен zero-coverage report (считаем, что весь код не покрыт тестами). 
